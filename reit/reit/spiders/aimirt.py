...
class AimirtSpider(scrapy.Spider):
    name = 'aimirt'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=AIMIRT&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=AIMIRT&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=AIMIRT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'AIMIRT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โครงการแปซิฟิคห้องเย็น แปซิฟิคห้องเย็น ห้องเย็นแปซิฟิค Pacific โครงการเจดับเบิ้ลยูดีแปซิฟิค เจดับเบิ้ลยูดีแปซิฟิค JWD โครงการดาต้าเซฟ ดาต้าเซฟ  DATASAFE โครงการทิพย์ 7 ทิพย์ TIP'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '22/12/2017'
            # Registration Date
            self.item['registrationDate'] = '09/01/2561'
