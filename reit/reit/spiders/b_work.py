...
class BWorkSpider(scrapy.Spider):
    name = 'b-work'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=B-WORK&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=B-WORK&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=B-WORK&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'B-WORK'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โครงการทรูทาวเวอร์ 1 อาคารทรู ทรู ตึกทรู ทรูทาวเวอร์ True Tower 1 โครงการทรูทาวเวอร์ 2 ทรูทาวเวอร์ 2 อาคารทรู 2 ตึกทรู 2 Tower 2'
            ...
            self.item['policy'] = response.css(
                "td div.row div.col-xs-12::text").getall()[54].strip()
            # Establishment Date
            self.item['establishmentDate'] = '6/2/2018'
            # Registration Date
            self.item['registrationDate'] = '28/02/2561'
