...
class FtreitSpider(scrapy.Spider):
    name = 'ftreit'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=FTREIT&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=FTREIT&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=FTREIT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'FTREIT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โครงการไทคอน ไทคอน ไทคอนโลจิสติคส์พาร์ค โครงการไทคอนโลจิสติคส์พาร์ค TICON Logistics Park'
            # Trustee Map To Elasticsearch
            self.item['trustee'] = 'บริษัทหลักทรัพย์จัดการกองทุนรวม บัวหลวง จำกัด'
            # Trust Name TH
            ...
            # Establishment Date
            self.item['establishmentDate'] = '12/12/2014'
            # Registration Date
            self.item['registrationDate'] = '09/01/2558'
