...
class GahreitSpider(scrapy.Spider):
    name = 'gahreit'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=GAHREIT&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=GAHREIT&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=GAHREIT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'GAHREIT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โรงแรมเชอราตัน เชอราตัน รีสอร์ทแอนด์สปา Sheraton Hua Hin Resort and Spa'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '10/10/2017'
            # Registration Date
            self.item['registrationDate'] = '03/11/2560'
