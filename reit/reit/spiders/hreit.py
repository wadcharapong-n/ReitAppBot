...
class HreitSpider(scrapy.Spider):
    name = 'hreit'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=HREIT&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=HREIT&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=HREIT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'HREIT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'นิคมอุตสาหกรรม ดับบลิวเอชเอ อีสเทิร์น ซีบอร์ด นิคมอุตสาหกรรมดับบลิวเอชเอ อีสเทิร์นซีบอร์ด นิคมอุตสาหกรรมดับบลิวเอชเออีสเทิร์น เขตประกอบอุตสาหกรรม SIL โลจิสติกส์พาร์ค Logistics Park'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '21/11/2016'
            # Registration Date
            self.item['registrationDate'] = '28/11/2559'
