...
class ImpactSpider(scrapy.Spider):
    name = 'impact'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=IMPACT&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=IMPACT&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=IMPACT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'IMPACT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'IMPACT Challenger Forum Arena Exhibition อิมแพ็คอารีน่า อิมแพ็คฟอรัม ศูนย์แสดงสินค้าอิมแพ็ค อิมแพ็คชาเลนเจอร์ อิมแพ็ค ชาเลนเจอร์ อารีน่า ฟอรัม'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '22/9/2014'
            # Registration Date
            self.item['registrationDate'] = '01/10/2557'
