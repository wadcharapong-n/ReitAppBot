...
class LhscSpider(scrapy.Spider):
    name = 'lhsc'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=LHSC&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=LHSC&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=LHSC&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'LHSC'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โครงการศูนย์การค้าเทอร์มินอล 21 ศูนย์การค้าเทอร์มินอล 21 เทอร์มินอล 21 Terminal 21 Mall'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '22/12/2014'
            # Registration Date
            self.item['registrationDate'] = '26/12/2557'
