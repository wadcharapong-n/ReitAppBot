...
class MitSpider(scrapy.Spider):
    name = 'mit'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=MIT&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=MIT&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=MIT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'MIT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = ''
            # Trustee Map To Elasticsearch
            self.item['trustee'] = 'บริษัทหลักทรัพย์จัดการกองทุนรวม วรรณ จำกัด'
            # Trust Name TH
            ...
            # Registration Date
            self.item['registrationDate'] = '23/12/2558'
