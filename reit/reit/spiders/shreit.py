...
class ShreitSpider(scrapy.Spider):
    name = 'shreit'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=SHREIT&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=SHREIT&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=SHREIT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'SHREIT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โรงแรมพัลแมนจาการ์ต้าเซ็นทรัลพาร์ค พัลแมนจาการ์ต้าเซ็นทรัลพาร์ค โรงแรมพัลแมน จาการ์ต้าเซ็นทรัล พัลแมน PULLMAN JAKARTA CENTRAL PARK HOTEL'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '20/12/2017'
            # Registration Date
            self.item['registrationDate'] = '27/12/2560'
