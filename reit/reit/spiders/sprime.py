...
class SprimeSpider(scrapy.Spider):
    name = 'sprime'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=SPRIME&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=SPRIME&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=SPRIME&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'SPRIME'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'โครงการซันทาวเวอร์ส ซันทาวเวอร์ส Sun Tower'
            ...
            self.item['policy'] = response.css(
                "td div.row div.col-xs-12::text").getall()[53].strip()
            # Establishment Date
            self.item['establishmentDate'] = '17/01/2019'
            # Registration Date
            self.item['registrationDate'] = '23/01/2562'
