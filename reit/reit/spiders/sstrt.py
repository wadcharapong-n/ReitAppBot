...
class SstrtSpider(scrapy.Spider):
    name = 'sstrt'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=SSTRT&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=SSTRT&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=SSTRT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'SSTRT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'เอสทีคลังสินค้า คลังสินค้าเอสที เอสที ST Warehouse คลังเอกสาร ทรัพย์ศรีไทย คลังเอกสารทรัพย์ศรีไทย Sap Sri'
            # Trustee Map To Elasticsearch
            self.item['trustee'] = 'บริษัทหลักทรัพย์จัดการกองทุน วรรณ จำกัด'
            # Trust Name TH
            ...
            # Establishment Date
            self.item['establishmentDate'] = '13/12/2017'
            # Registration Date
            self.item['registrationDate'] = '26/12/2560'
