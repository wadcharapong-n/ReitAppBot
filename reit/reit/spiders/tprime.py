...
class TprimeSpider(scrapy.Spider):
    name = 'tprime'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=TPRIME&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=TPRIME&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=TPRIME&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'TPRIME'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'อาคารเอ็กเชน ตึกเอ็กเชน เอ็กเชน Exchange Tower อาคารเมอร์คิวรี่ ตึกเมอร์คิวรี่ Mercury Tower'
            ...
            # Establishment Date
            self.item['establishmentDate'] = '11/10/2016'
            # Registration Date
            self.item['registrationDate'] = '31/10/2559'
