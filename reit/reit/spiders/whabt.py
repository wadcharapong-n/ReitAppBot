...
class WhabtSpider(scrapy.Spider):
    name = 'whabt'
    urls = ['https://www.set.or.th/set/companyprofile.do?symbol=WHABT&ssoPageId=4&language=th&country=TH',
            'https://www.set.or.th/set/companyprofile.do?symbol=WHABT&ssoPageId=4&language=en&country=TH',
            'https://marketdata.set.or.th/mkt/stockquotation.do?symbol=WHABT&ssoPageId=1&language=th&country=TH'
            ]
    item = ReitItem()
    one = False
    two = False
    ...
    def parse(self, response):
        ...
        if (response.url == self.urls[0]):
            print('Loop 0')
            # Symbol
            self.item['symbol'] = 'WHABT'
            # Nickname Map To Elasticsearch
            self.item['nickName'] = 'อาคารเอสเจ ตึกเอสเจ เอสเจ อินฟินิท วันบิสซิเนส SJ Infinite One Business Complex อาคารบางนา ตึกบางนา บิสซิเนสคอมเพล็กซ์ '
            # Trustee Map To Elasticsearch
            self.item['trustee'] = 'บริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)'
            # Trust Name TH
            ...
            # Establishment Date
            self.item['establishmentDate'] = '4/11/2015'
            # Registration Date
            self.item['registrationDate'] = '16/11/2558'
